...
def getTarget(row):
    return [row[col] for col in target_cols]


def getIndex(row, col):
    val = row[col].strip()
    if val not in ['', 'NA'] and mapping_dict[col].get(val):
...
def processData(data, cust_dict):
    ...
    if data['fecha_dato'] not in ['2015-05-28', '2015-06-28', '2016-05-28', '2016-06-28']:
        return x_vars_list, cust_dict

    cust_id = data['ncodpers']
    if data['fecha_dato'] in ['2015-05-28', '2016-05-28']:
        target_list = getTarget(data)
        cust_dict[cust_id] = target_list[:]
        return x_vars_list, cust_dict

    x_vars = []
    for col in cat_cols:
        x_vars.append(getIndex(data, col))
    x_vars.append(getAge(data))
    x_vars.append(getCustSeniority(data))
    x_vars.append(getRent(data))

    if data['fecha_dato'] == '2016-06-28':
        prev_target_list = cust_dict.get(cust_id, [0] * 22)
        x_vars_list.append(x_vars + prev_target_list)
    elif data['fecha_dato'] == '2015-06-28':
        prev_target_list = cust_dict.get(cust_id, [0] * 22)
        target_list = getTarget(data)
